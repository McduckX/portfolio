  ...
  const iconForLink = (label) => {
    ...
    if (label === "itch") {
      return (
        ...
        >
          <path
            d="M2 12C2 6.476 6.477 2 12 2s10 4.476 10 10-4.477 10-10 10S2 17.524 2 12Z"
            fill="#FA5C5C"
          />
          <path
            d="M7 12.5c0 1.933 1.566 3.5 3.5 3.5S14 14.433 14 12.5 12.434 9 10.5 9 7 10.567 7 12.5Zm5 0c0 .828-.672 1.5-1.5 1.5S9 13.328 9 12.5 9.672 11 10.5 11 12 11.672 12 12.5Z"
            fill="#fff"
          />
        </svg>
      );
  ...
  return (
    <article className="grid gap-8 lg:grid-cols-[1.2fr_1fr] items-start p-6 rounded-3xl border border-white/10 bg-white/5 shadow-xl shadow-black/20 transition hover:-translate-y-1 hover:border-purple-600/30">
      <div className="rounded-3xl border border-white/10 bg-black/80">
        <div className="relative overflow-hidden">
          {isVideo ? (
            ...
            <div className="flex h-[300px] items-center justify-center bg-slate-950/80 p-8 text-center text-sm text-gray-400">
              Replace this box with an optimized gameplay loop (.mp4 / .webm) or
              animated GIF.
            </div>
          )}
        </div>
        <div className="flex flex-wrap gap-3 border-t border-white/10 bg-slate-950/90 p-4">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-2xl bg-white/5 px-3 py-2 text-xs text-gray-300"
            >
      ...
      <div className="flex flex-col justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.28em] text-purple-400/80 mb-3">
            {releaseStatus}
          </p>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {platformTags?.map((tag) => (
              <span
                key={tag}
                className="rounded-full border border-white/10 bg-purple-600/10 px-3 py-1 text-xs text-purple-200"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex flex-wrap gap-3">
          {linkEntries.length > 0 ? (
            ...
            <span className="text-gray-500">
              Add storefront links for this project.
            </span>
          )}
          {onViewDetails && (
            <button
